...
function createAuth() {
  ...
  try {
    // Explicit persistence so session + profile survive offline cold starts
...
export const auth = isBrowser ? createAuth() : (undefined as unknown as ReturnType<typeof getAuth>);
